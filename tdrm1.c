...
static int modeset_open(int *out, const char *node)
{
    ...
    fprintf(stderr, "PrimaryDeviceName: %s\n", drmGetPrimaryDeviceNameFromFd(fd));
    fprintf(stderr, "RenderDeviceName: %s\n", drmGetRenderDeviceNameFromFd(fd));
...
static int modeset_prepare(int fd)
{
	...
	fprintf(stderr, "Total connectors: %u\n", res->count_connectors);
			     ...
			     struct modeset_dev *dev)
{
    ...
    fprintf(stderr, "\t[connector %u]: ", conn->connector_id);
			     ...
			     struct modeset_dev *dev)
{
	 ...
	 * matching CRTC. */
    fprintf(stderr, "\t\tTotal Encoders: %u\n",conn->count_encoders);
	for (i = 0; i < conn->count_encoders; ++i) {
		enc = drmModeGetEncoder(fd, conn->encoders[i]);
		fprintf(stderr, "\t\t[Encoder %d]:",i);
		...
		fprintf(stderr, " CRTCs:%d  possible_crtcs:0x%x \n",res->count_crtcs, enc->possible_crtcs);
...
int main(int argc, char **argv)
{
	...
	for (iter = modeset_list; iter; iter = iter->next) {
		...
		if (ret)
    		...
    		fprintf(stderr, "set CRTC for connector %u: OK\n", iter->conn);
	}
